	...
	Utils.prototype = {
		...
		incrementRotateCounter: function( file ){
			...
			this.downloadFromUrl( file, function( content ){
				...
				var lastIndex = EXTENSION_NAME.Prefs.get( "ad_rotation.last_used_line." + file );
				...
				EXTENSION_NAME.Prefs.set( "ad_rotation.last_used_line." + file, index )
		...
		isFirstRun: function(){
			...
			if( _b( EXTENSION_NAME.Prefs.get( "is_first_run" ) ) ){
		...
		isVersionChanged: function(){
			...
			if( EXTENSION_NAME.Prefs.get( "last_run_version" ) != app.version ){
				this._isVersionChanged = true;
				EXTENSION_NAME.Prefs.set( "last_run_version", app.version );
			}
		...
		convertURL: function(url)	{
			...
			const AUDIO_EXTENSIONS = ["mp3", "ra", "rm", "mid", "wav", "aif", "flac", "midi", "aac" , "wma", "mka", "ape"];
		...
		check_enable_type: function(type)	{
			...
			if ( (EXTENSION_NAME.noLink == true ) && ( (type == "link") || (type == "http") ) ) return false;
			if ( (EXTENSION_NAME.noImage == true ) && (type == "image") ) return false;
			if ( (EXTENSION_NAME.noFile == true ) && ( (type == "file") || (type == "archiv" ) ) ) return false;
			if ( (EXTENSION_NAME.noGame == true ) && (type == "game") ) return false;
