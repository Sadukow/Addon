...
function FVD_site_detector()
{
	...
	this.supported_sites_downloader = function(done_callback)
	{
		...
		ajax.open('GET', 'http://www.flashvideodownloader.org/fvd-suite/sites/sites.php', true);
	...
	this.is_supported = function(url)
	{
		...
		if (m != null)
		{
			...
			for (var i in this.sites)
			{
				if (i != 'RTMP')
				{
					if ((this.sites[i].indexOf(url) != -1) 
						&& (url.indexOf('youtube.com') == -1)
						 && (url.indexOf('dailymotion.com') == -1)
						) return true;
					if ((this.sites[i].indexOf(s_url) != -1) 
						&& (s_url.indexOf('youtube.com') == -1)
						 && (url.indexOf('dailymotion.com') == -1)
						) return true;
				}
			}
