	...
	init: function(){
		try
		{
			document.getElementById( "appcontent" ).addEventListener("DOMContentLoaded", function( event ){
				FVDSINGLE_Page.loadPageListener( event );
			}, true);
		}
		catch( ex ){
...
//			this.alert('href =' + href);
           	...
           	var script = this.getUrlContents('chrome://fvd.single/content/include/site_page.js');
	...
	insertScript: function( script, href, win ){
		...
		{
			dump( "FVD_Single: Expception while eval in sandbox - " + href + "("+ex+")\r\n" );
		}	
...
window.addEventListener("load", FVDSINGLE_Page.init, true);
