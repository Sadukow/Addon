...
FVDSINGLEDMButton = {
	...
	init: function(){
		try
		{
			document.getElementById( "appcontent" ).addEventListener("DOMContentLoaded", function( event ){
								FVDSINGLEDMButton.loadPageListener( event );
							}, true);
		}
		catch( ex )
		{
			dump( "Fail init DMButton listener. ("+ex+")\r\n" );
		}
	...
	loadPageListener: function( e ){
		...
		try
		{
			if( !fvd_single.registry.getBoolPref("display_youtube_button") )		return;
			...
			var regExp = new RegExp("http(s)?:\/\/.*\.dailymotion\.com\/");
			
			if( regExp.test( href ) )
			{				
            	var script = this.getUrlContents('chrome://fvd.single/content/include/dm_button_insert.js');
...
window.addEventListener("load", FVDSINGLEDMButton.init, true);
