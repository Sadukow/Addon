...
function createButton( buttonId, tooltip ){
	...
	button.setAttribute("style", "margin-left: -12px; border: solid;	border-right-width: 1px;	border-left-width: 1px;	border-top-width: 1px;	border-bottom-width: 1px;  border-color: #999;");
...
function createSetLi( menu, type )
{
	...
	if (type == 'formats')
	{
		...
		(function( span ){
				span.addEventListener("click", function(){
					FVDSINGLEYTButtonInsertor.SetFileTypes();
				}, true);
			})( span );
	}
	else if (type == 'folder')
	{
		...
		(function( span ){
				span.addEventListener("click", function(){
					FVDSINGLEYTButtonInsertor.SetDefautlFolder();
				}, true);
			})( span );
	}
	else if (type == 'rating')
	{
		...
		var span_t = document.createElement("span");
		span_t.setAttribute( "style", "margin-left: 10px;" );
		span_t.textContent = BUTTON_RATING_TITLE;
		span.appendChild(span_t);
		
		
		(function( span ){
				span.addEventListener("click", function(){
					FVDSINGLEYTButtonInsertor.GiveUsRating();
				}, true);
			})( span );
	}
	else if (type == 'betterfox')
	{
		...
		(function( span ){
				span.addEventListener("click", function(){
					FVDSINGLEYTButtonInsertor.Goto_Site('https://addons.mozilla.org/En-us/firefox/addon/betterfox/');
				}, true);
			})( span );
	}
...
function create_Like_Button(  menu )
{
	createSetLi( menu, 'rating' );
}
	
function buildMediaList( media, button, buttonId )
{
	...
	for( var k in media )
	{
		...
		var span_t = document.createElement("span");
		span_t.setAttribute( "style", "margin-left: 10px;" );
		span_t.textContent = m.raw_file_ext + " " + m.ext;
		span.appendChild(span_t);
		
		
		(function( medToDownload, span ){
			span.addEventListener("click", function(){
				FVDSINGLEYTButtonInsertor.startDownload( medToDownload );
			}, true);
		})( m, span );
	...
	createSetLi( menu, 'formats' );
	createSetLi( menu, 'folder' );
	FVDSINGLEYTButtonInsertor.GetLikeStatus(menu);
	createSetLi( menu, 'betterfox' );
	
	AD.getRotateItem(null, function( item ){
...
FVDSINGLEYTButtonInsertor = {
	
	BUTTON_TITLE: "Download",
	BUTTON_FORMATS_TITLE: "Set Default Formats",
	BUTTON_FOLDER_TITLE: "Set Default Dowlnoad folder",
	BUTTON_BETTERFOX_TITLE: "Make your Firefox 15% faster!",
	BUTTON_ID: "fvd_single_yt_download_button",
	...
	insert: function(  ){
		...
		if( !document.getElementById(this.BUTTON_ID) ){
			...
			var button = createButton( this.BUTTON_ID, this.BUTTON_TOOLTIP );
		...
		if( document.location.href.indexOf( "youtube.com/user/" ) != -1 ){
			function setupChannelDownloadButtons(){
				...
				for( var i = 0; i != videos.length; i++ )
				{
					...
					(function(video, videoId){
						...
						sniffer.getContentFromYoutubePage( url, videoId, function( media ){
							
							var buttonId = that.BUTTON_ID + "-" + videoId;
							...
							buildMediaList( media, button, buttonId );
	...
	mediaRequest: function( button ){
		var that = this;
		
		button = button || document.getElementById(FVDSINGLEYTButtonInsertor.BUTTON_ID);
		...
		this.sendAnonimouseEvent({
			...
			"callback": function( media ){
				...
				try{					
					buildMediaList( media, button, null );					
				}
				catch( ex ){
...
(function(){
	FVDSINGLEYTButtonInsertor.insert();	
})();