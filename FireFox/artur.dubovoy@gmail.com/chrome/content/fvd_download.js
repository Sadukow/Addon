	...
	FVD_SINGLE_DOWNLOAD = function(passiveMode)
	{
		...
		this.listener = {
			...
			onProgressChange : function(prog, req, prog, progMax, tProg, tProgMax, dl)
			{
				...
				{
					self.set_dl_button_progress( file.url, prog, progMax );	
				}
			},
...
/*								if( info.yt_id && !size && !clearYTCookieMessageDisplayed )
		...
		// -----------------------------------------------------------------	
		this.getActiveDownloads = function(){
			var dm = Components.classes['@mozilla.org/download-manager;1'].getService(Components.interfaces.nsIDownloadManager);
			var activeDownloads = dm.activeDownloads;
			
			var active = [];
			
			while( activeDownloads.hasMoreElements() )
			{
				var download = activeDownloads.getNext().QueryInterface(Components.interfaces.nsIDownload);
				active.push( download );
				//dump( "ACTIVE: " + download.state + ": " + download.source.spec + "\n" );
			}
		...
		this.init = function()		{	
			...
			if ('arguments' in window)
			{
				...
				var activeDownloads = this.getActiveDownloads();
				for( var i = 0; i != activeDownloads.length; i++ )
				{
					
					var activeDownload = activeDownloads[i];
					var activeDownloadUrl = activeDownload.source.spec;
				...
				this._setWindowContent( this.page_url, args.media, args.favicons );		
				
				if( args.sniffer )
				{
					this.sniffer = args.sniffer;
				}
			...
			var dm = Components.classes['@mozilla.org/download-manager;1'].getService(Components.interfaces.nsIDownloadManager);
			if (dm) dm.addListener(this.listener);		
		...
		this.uninit = function()
		{
			...
			var dm = Components.classes['@mozilla.org/download-manager;1'].getService(Components.interfaces.nsIDownloadManager);
			if (dm) dm.removeListener(this.listener);
		...
		this.downloadWindowsByUrl = function( page_url, url ){
			...
			var id = this.downloadByWindow( url, name, ext, referer );
			if( id )	this.downloadWindows[page_url].setDownloadId( url, id );
		...
		// -----------------------------------------------------------------	
		this.downloadByUrl = function( url ){
			...
			if( id ){
				...
				this._currentDownloadWindow().setDownloadId( url, id );
			}
		...
		// -----------------------------------------------------------------	
		this.downloadByWindow = function( url, name, ext, referer ){
			var bundle = Components.classes['@mozilla.org/intl/stringbundle;1'].getService(Components.interfaces.nsIStringBundleService).createBundle('chrome://fvd.single/locale/fvd.single.download.properties');
			...
			try{
				...
				{						
					var id = this.start_download_url(url, fp.fileURL, referer);
				...
				var id = this.start_download_url(url, uri, referer);
				return id;
			}
		...
		// -----------------------------------------------------------------	
		this.quickDownload = function( url, name, ext, referer ){
			...
			return this.start_download_url(url, ios.newFileURI(file), referer);
		...
		this.cancelDownload = function( url ){
			...
			try{
				var file = this._currentDownloadWindow().getFileByUrl( url );
				var dm = Components.classes['@mozilla.org/download-manager;1'].createInstance(Components.interfaces.nsIDownloadManager);
				dm.cancelDownload( file.downloadId );
...
/*			var name;
		...
		// -----------------------------------------------------------------	
		this.start_download_url_all = function(page_url, url, file_url, referer)
		{
			var media = {};
			...
			else
			{
				...
				if (from_url && to_url)
				{
					...
					var dm = Components.classes['@mozilla.org/download-manager;1'].createInstance(Components.interfaces.nsIDownloadManager);
					var dl = dm.addDownload(dm.DOWNLOAD_TYPE_DOWNLOAD, from_url, to_url, '', null, Math.round(Date.now() * 1000), null, persist, null);
					...
					var privacyContext = sourceWindow.QueryInterface(Components.interfaces.nsIInterfaceRequestor).getInterface(Components.interfaces.nsIWebNavigation).QueryInterface(Components.interfaces.nsILoadContext);					
					persist.saveURI(dl.source, null, refererUrl, null, null, dl.targetFile, privacyContext);
		...
		// -----------------------------------------------------------------	
		this.quickDownloadAll = function( page_url, url, name, ext, referer ){
			...
			return this.start_download_url_all(page_url, url, ios.newFileURI(file), referer);
		}
		...
		// ------------------------------------------------------------------- реакци€ на —качать все медиа файлы
			...
			for( page_url in this.downloadWindows )
			{
				...
				for (var i in windowMedia)
				{
					...
					if (info['direct'])
					{
						...
						{
							if( this.check_default_format( info['ext'], page_url ) )  
							{
								var label;
								...
								var downloadId = this.quickDownloadAll(page_url, info.url, label, "." + info.ext, info.referer);		      // без запроса			
//								var downloadId = this.downloadWindowsByUrl( page_url, info.url );                             // с запросом папки

								if( downloadId )
								{
									this.downloadWindows[page_url].setDownloadId( info.url, downloadId );
								}
							}					
						}
		...
		this.download_all_current = function(event)
		{
			...
			for (var i in windowMedia)
			{
				...
				if (info['direct'])
				{
					if ( !info.downloadId ) // skip all processed tasks
					{
						if( this.check_default_format( info['ext'], info.url ) )  
						{
		...
		// -----------------------------------------------------------------	
		this.start_download_url = function(url, file_url, referer)
		{
			var media = {};
		...
		this.set_dl_button_progress = function( url, progress, maxProgress ){
			...
			if (list) {
				...
				if( button )
				{
					...
					button.setAttribute( "label", bundle.GetStringFromName('fvd.single.downloading.txt') + percents+"%)" );
				}
			}
		}
