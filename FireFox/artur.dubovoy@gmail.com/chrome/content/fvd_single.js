...
{	
		...
		for (var page_url in media)
		{
			...
			{				
				...
				if ( /*!this.detector.is_supported(url)*/ true )
				{
					...
					if (mediaFile['ext'])		it['ext'] = mediaFile['ext'];
					
					if ( "file_title" in mediaFile )		it.file_title = mediaFile.file_title;
					
					if ( "raw_file_ext" in mediaFile )		it.raw_file_ext = mediaFile.raw_file_ext;
					
					if ( "download_name" in mediaFile )		it.download_name = mediaFile.download_name;
					
					if ( "playable" in mediaFile )			it.playable = mediaFile.playable;
					
					if ( "type" in mediaFile )				it.type = mediaFile.type;
					
					if ( "height" in mediaFile )			it.height = mediaFile.height;
					
					if ( "referer" in mediaFile )			it.referer = mediaFile.referer;
	
					if ( "size" in mediaFile )				it.size = mediaFile.size;
					
					if( !it.node )
					{
						if( pageVideos.length == 1 )		it.node = pageVideos[0];
						else if( pageEmbeds.length == 1 )	it.node = pageEmbeds[0];
						else if( pageObjects.length == 1 )	it.node = pageObjects[0];
						else if( pageIframes.length == 1 )	it.node = pageIframes[0];
					}
					
					if( !(page_url in result) )			result[page_url] = {};
	...
	// ------------------------------------------------------------------------------------------
 	this.extendMediaByUrl = function( media, extendData, url ){				
		...
		for( var mediaUrl in extendData )		media[url][mediaUrl] = extendData[ mediaUrl ];
	...
	this.parseFavicons = function( media ){
		...
		for( var pageUrl in media )
		{
			...
			else
			{
				...
				if( doc )		favicons[pageUrl] = this.parseFaviconUrl(doc);					
				else			favicons[pageUrl] = "";
	...
	// ------------------------------------------------------------------------------------------ нажали на иконку на панели задач
		...
		try
		{
			if (this.supported)
			{
				...
				else
				{
					...
					// считаем из сниффера данные по media
					var sniffedMedia = this.prepareSniffedMedia( doc, this.sniffer.get_files_all() );		
					...
					for( var i in media )
					{
						...
						for( k in m )
						{
							if( !this.check_shows_format( m[k].ext, m[k].url ) )  continue;
						
							var elem = document.getElementById("fvd_single_fast_menu_item_prototype").cloneNode( true ); 
...
//			if (!this.is_tab_page(i)) continue;
	...
	this.browser_progress_listener = {
		...
		// -- Вызывается, когда URI документа отображается на вкладке изменения
		...
		// -- Уведомление, указывающее состояние изменилось на один из запросов, связанных с aWebProgress .				
		onStateChange: function(aWebProgress, aRequest, aStateFlags, aStatus) {

						},
						
		// -- Вызывается, когда обновляется информация о ходе выполнения для загрузки документа доступен					
		onProgressChange:  function(aWebProgress, aRequest, aCurSelfProgress, aMaxSelfProgress, aCurTotalProgress, aMaxTotalProgress) {
		
						},
						
		// -- Уведомление о статусе запроса изменилась					
		onStatusChange: function(aWebProgress, aRequest, aStatus, aMessage) {
			
						},
						
		// -- Этот метод будет вызван безопасности переходы (например, HTTP -> HTTPS, HTTPS -> HTTP)				
		onSecurityChange: function(aWebProgress, aRequest, aState) {
		
						},
						
		// -- Уведомление о том, иконку сайта для браузера был найден				
	...
	this.document_embed_check = function()
	{
		...
		try
		{
			...
			if (doc)
			{
				...
				if( !this.is_youtube(doc) )		return false;
...
/*	this.contextButtonClick = function( event ){
	...
	// ------------------------------------------------------------------------------------------  проверить наличие иконки а панели задач
	...
	// ------------------------------------------------------------------------------------------  проверить наличие иконки а панели дополнений
	...
	this.status_button_insert = function(){
		if( document.getElementById("fvd_single_status") )		return;
	...
	// ------------------------------------------------------------------------------------------  удалить иконку из панели дополнений
	this.status_button_remove = function(){
		...
		if (addonBar) 
		{
			...
			{
				if( item == "fvd_single_status" )		continue;
	...
	this.is_first_start = function(callback, anyWayCallback)
	{
		...
		{						
			...
			AddonManager.getAddonByID(EXTENSION_GUID, function(addon) {				
							...
							if( anyWayCallback )
							{
								...
								if( isFirstRun )		self.registry.setBoolPref('is_first_run', false);
		...
		else
		{
			...
			try
			{
				var loc = Components.classes['@mozilla.org/extensions/manager;1'].getService(Components.interfaces.nsIExtensionManager).getInstallLocation(EXTENSION_GUID).location;
	...
	this.settings_observer_struct = {
		observe : function(aSubject, aTopic, aData) 	{
				switch ( aTopic )
				{
					...
					{
						//fvd_single_AD.getRotateItem(null, function( item ){			fvd_single.button_Youtube.insert( item );		} );
						fvd_single.button_Youtube.insert( null );		
						break;
					}
			
					case 'FVD.Single-Media-DailyMotion':
					{
						//fvd_single_AD.getRotateItem(null, function( item ){				fvd_single.button_DailyMotion.insert( item );	} );
						fvd_single.button_DailyMotion.insert( null );	
						break;
					}
			
					case 'FVD.Single-Media-VKontakte':
					{
	...
	this.init = function()
	{
		...
		{
			dump("INIT detector - sniffer - page  " + ex + "\r\n");
		};
		...
		this.observer.addObserver(this.settings_observer_struct, 'FVD.Single-Media-VKontakte', false);
		...
		this.is_first_start(function()
							...
							function( isFirstStart, isFirstRun ){
											if( isFirstStart )
											{
												...
												if( isFirstRun )
												{
													self.main_button_insert();			// кнопка в панели задач пояляется сразу
													//self.status_button_insert();		// кнопка в панели дополнений только по установке
