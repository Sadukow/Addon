	...
	FVD_SINGLE_DM_Button = function(  ){
			...
			if( !doc.getElementById(this.BUTTON_ID) ){
				...
				button.addEventListener("click", function(event){
								...
								if( media[location.spec] )
								{
									...
									self.buildMediaListDM( media, button, null, item );					
								}	
								
							}, false);
			
				var elemFlag = doc.getElementsByClassName('social_bar')[0];
				if( elemFlag )
				{
					var elemB = doc.getElementsByClassName('sd_video_socialbuttons')[0];
				...
				{
					elemFlag = doc.getElementById('dmpi_video_tools');
					var dv = doc.createElement("li");
					dv.appendChild( button );	
		...
		this.createSetLi = function( menu, type )  {
			...
			else if (type == 'betterfox')
			{
				...
				var img = doc.createElement("img");
				img.setAttribute("align","left");
				img.setAttribute("src", EXT_IMAGES["bet"]);
				span.appendChild(img);
