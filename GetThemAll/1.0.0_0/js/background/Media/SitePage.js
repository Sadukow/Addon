		...
		this.check_YT_User = function( tabId, answer, url, link, callback ){
			...
			for (var i = 0; i < link.length; i++) 
			{
				if( link[i].type == "video" )
				{
					...
					if ( availFormats )
					{	
						var pMedia = get_youtube_format( availFormats, fileName, length_seconds );
						pMedia.forEach(function( m ){
...
//console.log(obj);								
/*								var mm = obj.sequence[0].layerList[0].sequenceList;						
								for (var jj = 0; jj < mm.length; jj++) 
								{
									if ( mm[jj].name == "main")
									{
										var nn = mm[jj].layerList[0].sequenceList[0].layerList[0].param.qualities;
...
//console.log(unescapeFromUtf16(data.videotitle));						
...
//				callback( { command: "Get_Links", tip: "link, image, input", answer: "vk_audio", tabId: tab.id } );
			...
			{
				EXTENSION_NAME.Media.Storage.removeTabSourceData( tab.id, "SitePage" );
				callback( { command: "Get_Links", tip: " image, embed", answer: "yt_video", tabId: tab.id } );
				return "yt_video";
			}
