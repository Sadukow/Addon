	...
	if (curHref != url) 
	{
		...
		chrome.extension.sendRequest({akce:"Page_URL",  url: url  },	
										function( response ){ 	
											...
											else if ( response.command == 'Get_YT_User')
											{
												getYT_User_URLs( response.tip, response.answer, response.tabId );
											}
									} );
	}								
}
...
// ---------------------------------------------------------------------------
function getYT_User_URLs( tip, answer, tabId ) {
	...
	chrome.extension.sendRequest({akce:"Get_YT_User", answer: answer, tabId: tabId, url: url, link: aURLs   },	
												function( response ){ 	
											} );
